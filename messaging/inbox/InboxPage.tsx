    ...
    const fetch = async () => {
      try {
        ...
        if (mounted && !error) {
          ...
          if (data?.[0]) setActiveConv(data[0]);
        }
      } finally {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-10 h-[85vh] flex gap-8">
      ...
      <div className="w-96 bg-white dark:bg-[#0c0c0e] rounded-[2.5rem] border dark:border-white/5 flex flex-col overflow-hidden shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.map(c => (
            ...
            >
              <img src={c.listing?.image_url as string} className="w-12 h-12 rounded-xl object-cover" />
              <div className="min-w-0 flex-1">
                <p className="font-black dark:text-white text-sm truncate">{c.listing?.title}</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase truncate">Trade with {c.seller_id === user.id ? c.buyer?.full_name : c.seller?.full_name}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 bg-white dark:bg-[#0c0c0e] rounded-[2.5rem] border dark:border-white/5 flex flex-col overflow-hidden shadow-xl">
        {activeConv ? (
          <>
            <div className="p-6 border-b dark:border-white/5 bg-gray-50/10 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-indigo-500">{activeConv.listing?.price} ETB</p>
              </div>
            </div>
            ...
            <form onSubmit={send} className="p-8 border-t dark:border-white/5 flex gap-4">
              ...
              <button className="bg-black dark:bg-white text-white dark:text-black px-10 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl">Send</button>
            </form>
          </>
        ) : (
