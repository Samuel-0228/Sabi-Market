    ...
    <nav className="sticky top-0 z-50 glass border-b border-white/10 px-6 sm:px-10 py-4 sm:py-5 flex items-center justify-between transition-all duration-700 dark:border-white/5">
      <div className="flex items-center gap-6 sm:gap-12">
        <div className="flex items-center gap-4 cursor-pointer group" onClick={() => onNavigate('landing')}>
          <div className="w-10 h-10 bg-black dark:bg-white rounded-2xl flex items-center justify-center text-white dark:text-black font-bold text-xl shadow-xl group-hover:scale-110 transition-all">
            ሳ
          </div>
      ...
      <div className="flex items-center gap-4">
        <button onClick={toggleTheme} className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center hover:scale-110 transition-all text-xl">
          {theme === 'light' ? '🌙' : '☀️'}
        </button>

        <button onClick={() => setLang(lang === 'en' ? 'am' : 'en')} className="bg-gray-100 dark:bg-white/5 px-4 py-2 rounded-xl text-[10px] font-black text-black dark:text-white uppercase tracking-[0.1em] transition-all">
        ...
        {user ? (
          <div className="flex items-center gap-4">
            ...
            <button onClick={onLogout} className="w-10 h-10 bg-gray-50 dark:bg-white/5 hover:bg-red-500 hover:text-white rounded-xl flex items-center justify-center transition-all">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
            </button>
          </div>
        ) : (
          <button onClick={() => onNavigate('login')} className="bg-black dark:bg-white text-white dark:text-black px-6 py-3 rounded-xl text-[10px] font-black tracking-widest uppercase shadow-xl transition-all">
            {t('login')}
          </button>
        )}
      </div>
