    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4 animate-in fade-in">
      <div className="bg-white dark:bg-[#0c0c0e] w-full max-w-5xl rounded-[3rem] overflow-hidden flex flex-col md:flex-row h-full max-h-[90vh] shadow-2xl border border-white/5">
        <div className="md:w-[40%] bg-gray-50/50 dark:bg-black/20 p-10 flex flex-col items-center justify-center border-r dark:border-white/5">
          ...
          <input ref={fileInputRef} type="file" className="hidden" accept="image/*" onChange={handleFileUpload} title="Upload photo" aria-label="Upload photo" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-12 flex-1 flex flex-col overflow-y-auto">
          <div className="flex justify-between items-center mb-10">
            ...
            <button type="button" onClick={onClose} aria-label="Close add listing modal" title="Close" className="w-12 h-12 bg-gray-50 dark:bg-white/10 rounded-full flex items-center justify-center hover:bg-red-500 transition-all">✕</button>
          </div>

          <div className="space-y-6 flex-1">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Product Title</label>
              <input required title="Product title" className="w-full bg-gray-50 dark:bg-white/5 border-none p-5 rounded-2xl dark:text-white font-bold outline-none focus:ring-2 focus:ring-indigo-600" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
            </div>

            <div className="grid grid-cols-2 gap-6">
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Price (ETB)</label>
                 <input required type="number" title="Price" className="w-full bg-gray-50 dark:bg-white/5 border-none p-5 rounded-2xl dark:text-white font-bold outline-none" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} />
               </div>
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Category</label>
                 <select title="Category" className="w-full bg-gray-50 dark:bg-white/5 border-none p-5 rounded-2xl dark:text-white font-bold outline-none" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
            ...
            <div className="space-y-2">
              ...
              <textarea required rows={4} title="Description" className="w-full bg-gray-50 dark:bg-white/5 border-none p-5 rounded-2xl dark:text-white font-bold outline-none resize-none" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
            </div>
          </div>
