...
const OrdersPage: React.FC<OrdersPageProps> = ({ user }) => {
  ...
  const handleConfirmReceipt = async (orderId: string) => {
    if (!confirm("Confirm delivery? This will release the escrow funds to the seller.")) return;
    try {
  ...
  const handleRatingSubmit = async (rating: number, comment: string) => {
    ...
    try {
      await ratingService.createRating({
        ...
        reviewer_id: user.id,
        reviewee_id: ratingOrder.seller_id,
        rating,
        comment
      });
      alert("Rating submitted! Thank you for building trust in AAU.");
    } catch (error) {
  ...
  if (loading && orders.length === 0) return (
    <div className="h-screen flex items-center justify-center bg-savvy-surface">
      <div className="w-8 h-8 border-2 border-savvy-accent border-t-transparent rounded-full animate-spin"></div>
    </div>
  );

  return (
    <div className="bg-savvy-surface min-h-screen max-w-[1200px] mx-auto px-4 md:px-10 py-24 md:py-40 pb-32">
      <header className="mb-8 md:mb-16 reveal">
        ...
        <h1 className="text-3xl md:text-7xl font-black text-savvy-text tracking-tighter uppercase">Purchases.</h1>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
        {orders.length === 0 ? (
          <div className="col-span-full py-20 text-center opacity-30 reveal">
            <p className="text-sm font-black uppercase tracking-widest text-savvy-text">No active acquisitions</p>
          </div>
        ) : (
          orders.map((order) => (
            <div key={order.id} className="reveal bg-white dark:bg-[#0c0c0e] rounded-2xl tibico-border p-4 flex gap-4 items-center shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between mb-1">
                  <h3 className="text-[11px] font-black text-savvy-text tracking-tight truncate leading-tight pr-2">{order.product_title}</h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] font-bold text-savvy-text truncate">{order.seller_name}</p>
                  </div>
                  {(order.status === 'accepted' || order.status === 'shipped' || order.status === 'delivered') && (
                    <button onClick={() => handleConfirmReceipt(order.id)} className="px-3 py-1.5 bg-savvy-text text-savvy-surface rounded-lg text-[7px] font-black uppercase tracking-widest shadow-md">
                      Confirm Receipt
                    </button>
                  )}
                  {order.status === 'completed' && (
                    <button onClick={() => setRatingOrder(order)} className="px-3 py-1.5 border border-savvy-accent text-savvy-accent rounded-lg text-[7px] font-black uppercase tracking-widest">
                      Rate Seller
                    </button>
                  )}
                </div>
      ...
      <RatingModal
        ...
        title="Rate Seller"
        subtitle={`How was your experience with ${ratingOrder?.seller_name}?`}
      />
    </div>
  );
};
